...
export function discoveredMcpTools(messages: MessageV2.WithParts[]) {
  return new Set(
    messages.flatMap((message) =>
      message.parts.flatMap((part) => {
        if (part.type !== "tool" || part.tool !== MCP_SEARCH_TOOL || part.state.status !== "completed") return []
        const metadata = asStringArray(part.state.metadata.discoveredTools)
        if (metadata.length) return metadata
        try {
          const parsed = JSON.parse(part.state.output) as Record<string, unknown>
          return asStringArray(parsed.discovered_tools).concat(
            Array.isArray(parsed.results)
              ? parsed.results.flatMap((item) =>
...
export function searchMcpTools(input: { tools: McpToolSearchItem[]; query: string; limit?: number }) {
  ...
  const scored = input.tools
    .map((item) => {
      ...
      const score = terms.reduce((sum, term) => {
        if (id === term) return sum + 20
        if (id.includes(term)) return sum + 10
        if (description.includes(term)) return sum + 3
        return sum
      }, 0)
      return { ...item, score }
    })
...
## Read-Only Exploration Rules
- Prefer read, glob, grep, and explore agents for codebase understanding.
- Bash may only be used for inspection commands such as listing files, reading metadata, checking git status or diffs, and harmless discovery.
- Do not create temporary files, redirect output to files, install dependencies, run formatters, or execute commands that mutate project state.
...
- Include assumptions, risks, and tradeoffs that could affect implementation
- Include a sequenced implementation checklist
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
    ...
    const resolveTools = Effect.fn("SessionPrompt.resolveTools")(function* (input: {
      ...
      const mcpTools = Object.entries(yield* mcp.tools()).filter(([key]) => Agent.allowsTool(input.agent, key))
      const cfg = yield* config.get()
      const discovered = discoveredMcpTools(input.messages)
      const mcpSearchEnabled =
        cfg.experimental?.mcp_tool_search !== false &&
        mcpTools.length > (cfg.experimental?.mcp_tool_search_threshold ?? MCP_SEARCH_THRESHOLD) &&
        Agent.allowsTool(input.agent, MCP_SEARCH_TOOL)
      yield* debug.event({
        component: "session.prompt",
        event: "resolve_tools",
        data: {
          agent: input.agent.name,
          sessionID: input.session.id,
          modelID: input.model.api.id,
          builtinToolCount: Object.keys(tools).length,
          mcpToolCount: mcpTools.length,
          mcpSearchEnabled,
          discoveredMcpTools: [...discovered],
        },
      })
      if (mcpSearchEnabled) {
        tools[MCP_SEARCH_TOOL] = tool({
          description:
            "Search available MCP tools by name and description. Use this when you need an external MCP capability that is not currently exposed.",
          inputSchema: jsonSchema({
            type: "object",
            properties: {
              query: { type: "string", description: "Search terms describing the needed MCP capability." },
              limit: { type: "integer", minimum: 1, maximum: 25, description: "Maximum number of results to return." },
            },
            required: ["query"],
          ...
          async execute(args) {
            ...
            const results = searchMcpTools({
              tools: mcpTools.map(([id, item]) => ({ id, description: item.description ?? "" })),
              query,
              limit,
            })
            return {
              title: "MCP Tool Search",
              output: JSON.stringify({ results, discovered_tools: results.map((item) => item.id) }, null, 2),
              metadata: { discoveredTools: results.map((item) => item.id) },
            }
          },
      ...
      for (const [key, item] of mcpTools.filter(([key]) => !mcpSearchEnabled || discovered.has(key))) {
        ...
        item.execute = (args, opts) =>
          run.promise(
            Effect.gen(function* () {
              ...
              yield* debug.event({
                component: "session.prompt",
                event: "mcp_tool.execute.start",
                data: { tool: key, sessionID: ctx.sessionID, callID: opts.toolCallId },
              })
              yield* plugin.trigger(
                "tool.execute.before",
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const [skills, env, agentContext, instructions, modelMsgs] = yield* Effect.all([
              sys.skills(agent),
              sys.environment(model),
              sys.agentContext(agent),
              agent.options?.omitInstructions === true
                ? Effect.succeed([] as string[])
                : instruction.system().pipe(Effect.orDie),
              MessageV2.toModelMessagesEffect(msgs, model),
            ])
            const system = [...env, ...agentContext, ...instructions, ...(skills ? [skills] : [])]
...
).pipe(Layer.provide(Debug.defaultLayer))
