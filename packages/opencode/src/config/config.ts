...
export const Info = Schema.Struct({
  ...
  model_alias: Schema.optional(Schema.Record(Schema.String, Schema.String)).annotate({
    description: "Model alias mappings. Keys are short names, values are full provider/model strings.",
  }),
  ...
  experimental: Schema.optional(
    Schema.Struct({
      ...
      mcp_tool_search: Schema.optional(Schema.Boolean).annotate({
        description:
          "When true, expose an MCP search tool instead of every MCP tool once the MCP tool count exceeds the configured threshold.",
      }),
      mcp_tool_search_threshold: Schema.optional(PositiveInt).annotate({
        description: "Number of MCP tools allowed before MCP tool search is used (default: 16).",
      }),
    }),
  ),
