...
test("searchMcpTools ranks matching MCP tools by id and description", () => {
  expect(
    SessionPrompt.searchMcpTools({
      query: "jira issue",
      tools: [
        { id: "mcp_slack_post_message", description: "Post a chat message" },
        { id: "mcp_jira_search", description: "Search issue tracker tickets" },
        { id: "mcp_docs_lookup", description: "Look up documentation" },
      ],
    }),
  ).toEqual([{ id: "mcp_jira_search", description: "Search issue tracker tickets" }])
})

test("discoveredMcpTools reads MCP search metadata and output fallbacks", () => {
  const sessionID = SessionID.make("ses_mcp_search")
  const messageID = MessageID.ascending()
  const parentID = MessageID.ascending()
  expect(
    SessionPrompt.discoveredMcpTools([
      {
        info: {
          id: messageID,
          parentID,
        ...
        parts: [
          {
            ...
            state: {
              status: "completed",
              input: {},
              output: JSON.stringify({ discovered_tools: ["mcp_docs_lookup"] }),
              title: "MCP Tool Search",
              metadata: { discoveredTools: ["mcp_jira_search"] },
  ...
  ).toEqual(new Set(["mcp_jira_search"]))
})
