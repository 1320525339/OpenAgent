...
describe("tool.task", () => {
  ...
  background.instance(
    "agent background config launches background tasks by default",
    () =>
      Effect.gen(function* () {
        const jobs = yield* BackgroundJob.Service
        const { chat, assistant } = yield* seed()
        ...
        const result = yield* def.execute(
          {
            description: "inspect bug",
            prompt: "look into the cache key path",
            subagent_type: "background_reviewer",
          },
          {
            sessionID: chat.id,
            messageID: assistant.id,
  ...
  background.instance(
    "task background parameter can override agent background config",
    () =>
      Effect.gen(function* () {
        const { chat, assistant } = yield* seed()
        const tool = yield* TaskTool
