...
const debugRegistry = testEffect(Layer.mergeAll(registryLayer(), node, Agent.defaultLayer, Debug.defaultLayer))
...
describe("tool.registry", () => {
  ...
  debugRegistry.instance("records registry events in the active debug session", () =>
    Effect.gen(function* () {
      const debug = yield* Debug.Service
      const registry = yield* ToolRegistry.Service
      const agents = yield* Agent.Service
      yield* debug.bootstrap({ mode: "trace", bugType: "registry-test" })
      ...
      expect(report.components).toContain("debug")
      expect(report.components).toContain("tool.registry")
      expect(report.recentEvents.some((event) => event.event === "tools.resolved")).toBe(true)
      yield* debug.cleanup({ reason: "test complete" })
    }),
  )
