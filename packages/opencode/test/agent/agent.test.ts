...
it.instance("verification agent is read-only and runnable as a subagent", () =>
  Effect.gen(function* () {
    const verification = yield* load((svc) => svc.get("verification"))
    expect(verification).toBeDefined()
    expect(verification?.mode).toBe("subagent")
    expect(verification?.background).toBe(true)
    expect(verification?.prompt).toContain("VERDICT: PASS")
    expect(evalPerm(verification, "read")).toBe("allow")
    expect(evalPerm(verification, "bash")).toBe("allow")
    expect(evalPerm(verification, "edit")).toBe("deny")
    expect(evalPerm(verification, "write")).toBe("deny")
    expect(Agent.allowsTool(verification!, "read")).toBe(true)
    expect(Agent.allowsTool(verification!, "bash")).toBe(true)
    expect(Agent.allowsTool(verification!, "edit")).toBe(false)
    expect(Agent.allowsTool(verification!, "write")).toBe(false)
    expect(Agent.allowsTool(verification!, "task")).toBe(false)
  }),
)
...
it.instance(
  "custom agent supports runtime tool and skill boundaries",
  () =>
    Effect.gen(function* () {
      const reviewer = yield* load((svc) => svc.get("reviewer"))
      expect(reviewer?.permissionMode).toBe("ask")
      expect(reviewer?.allowedTools).toEqual(["read", "grep", "task"])
      expect(reviewer?.disallowedTools).toEqual(["task"])
      expect(reviewer?.skills).toEqual(["code-review"])
      expect(reviewer?.background).toBe(true)
      expect(reviewer?.memoryScope).toBe("project-review")
      expect(evalPerm(reviewer, "read")).toBe("allow")
      expect(evalPerm(reviewer, "edit")).toBe("deny")
      expect(evalPerm(reviewer, "bash")).toBe("allow")
      expect(Agent.allowsTool(reviewer!, "read")).toBe(true)
      expect(Agent.allowsTool(reviewer!, "task")).toBe(false)
      expect(Agent.allowsTool(reviewer!, "write")).toBe(false)
      expect(Agent.allowsSkill(reviewer!, "code-review")).toBe(true)
      expect(Agent.allowsSkill(reviewer!, "design")).toBe(false)
    }),
  {
    config: {
      agent: {
...
it.instance(
  "defaultAgent returns ask when build is disabled and default_agent not set",
  () =>
    Effect.gen(function* () {
      const agent = yield* load((svc) => svc.defaultAgent())
